...
(async () => {
  ...
  let mdCont = '';
  if (myArgs[0].includes("http")) {
    const resp = await fetch(myArgs[0]);
    mdCont = await resp.text();
  } else {
    mdCont = fs.readFileSync(myArgs[0], {encoding:'utf8', flag:'r'});
  }
      ...
      value.forEach((row) => {
        ...
        if (row.typ.includes('#')) {
          ...
          if (row.typ.includes('[object map]')) {
            row.typ = 'json';
          } else if (row.typ.includes('reference data table to describe what this record is.')) {
